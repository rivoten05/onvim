local function execute_lua_chunk(chunk)
    ...
    vim.api.nvim_open_win(buf, true, {
        ...
        style = "minimal", border = "rounded", title = " Lua Runner ", title_pos = "center",
    })
    ...
    print = function(...)
        ...
        for _, l in ipairs(lines) do table.insert(output, l) end
    end
    ...
    if func then ok, result = pcall(func) else ok, result = false, err end

    local duration = (vim.loop.hrtime() - start_time) / 1e6
    print = old_print

    if ok then
        if result ~= nil then table.insert(output, "Result: " .. vim.inspect(result))
        else table.insert(output, "✔ Executed successfully") end
    else table.insert(output, "❌ Error: " .. result) end
...
vim.keymap.set("n", "<leader>bx", eval_buffer, { desc = "Run Lua Buffer" })
vim.keymap.set("v", "<leader>bx", eval_region, { desc = "Run Lua Selection" })
